      ...
      <div className={`flex h-screen w-full transition-all duration-300 overflow-hidden ${darkMode ? 'bg-[#0f172a] text-white' : 'bg-[#f4f7fb] text-slate-900'}`}>
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden relative">
          <header className="px-8 py-6 flex justify-between items-start shrink-0">
            <div><h2 className="text-2xl font-extrabold flex items-center gap-2">Master Ledger 📋</h2><p className={`mt-1 text-sm font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Manage all production entries, columns, and history logs.</p></div>
          ...
          <div className="flex-1 px-8 pb-8 overflow-hidden flex flex-col">
            ...
            <div className={`transition-all rounded-2xl shadow-sm overflow-hidden flex-1 flex flex-col border ${darkMode ? 'bg-[#1e293b] border-gray-800' : 'bg-white border-gray-100'}`} style={{ height: '100%', minHeight: 0 }}>
              ...
              {/* Added tableStyle 120rem to allow proper horizontal flow without overriding strict columns */}
              <DataTable
                value={data}
                ...
                style={{ height: '100%' }}
              >
                {/* STRICT WIDTHS APPLIED HERE for ALL frozen columns to fix the alignment issue */}
                {isAdmin && <Column selectionMode="multiple" frozen alignFrozen="left" style={{ minWidth: '50px', width: '50px', maxWidth: '50px' }} />}

                <Column field="catNo" header="CAT NO" sortable filter body={createClickableBody('catNo')} frozen alignFrozen="left" style={{ minWidth: '160px', width: '160px', maxWidth: '160px' }} />
                
                <Column field="styleNo" header="Style No." sortable filter body={createClickableBody('styleNo')} frozen alignFrozen="left" style={{ minWidth: '180px', width: '180px', maxWidth: '180px' }} />
                
                <Column field="factoryFOB" header="Factory FOB" sortable filter body={createClickableBody('factoryFOB', true)} style={{ minWidth: '160px' }} />
                ...
                <Column body={actionBodyTemplate} header="Edit" frozen alignFrozen="right" style={{ minWidth: '80px', width: '80px', maxWidth: '80px' }} />
              </DataTable>
            </div>
          </div>
        </main>
