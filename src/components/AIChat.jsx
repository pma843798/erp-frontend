...
const AIChat = ({ embedded = false }) => {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            setMessages((prev) => [...prev, { role: 'ai', text: '⚠️ Something went wrong.' }]);
        }
        setLoading(false);
    };
    ...
    // ---- Embedded mode: just return the chat panel (no floating button, no Rnd, no outer toggle) ----
    if (embedded) {
        return (
            <div className="flex flex-col h-full w-full bg-gradient-to-b from-blue-50 to-white dark:from-gray-900 dark:to-gray-800">
                {/* Chat messages */}
                <div className="flex-1 overflow-y-auto px-4 py-5 space-y-4">
                                ...
                                <div
                                    className={`max-w-[88%] px-4 py-3 rounded-2xl text-[13px] leading-relaxed shadow-md border ${
                                        msg.role === 'user'
                                            ...
                                            : 'bg-white dark:bg-gray-800 text-gray-800 dark:text-white border-gray-200 dark:border-gray-700 rounded-bl-md'
                                    }`}
                                >
                    ...
                    {loading && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
                            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-4 py-3 rounded-2xl flex items-center gap-2 shadow-sm">
                ...
                {/* Input form */}
                <form onSubmit={handleSend} className="p-4 border-t border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-900/50 backdrop-blur-xl">
                    <div className="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-3 py-2 shadow-sm">
                        <input
                            type="text"
                            value={input}
                            onChange={(e) => setInput(e.target.value)}
                            placeholder="Ask PMA Smart System..."
                            className="flex-1 bg-transparent outline-none text-gray-800 dark:text-white placeholder:text-gray-500 text-sm"
                        />
                        <motion.button
    ...
    // ---- Standalone mode: floating button + draggable/resizable window ----
    if (!isOpen) {
        return (
            <motion.button
                initial={{ scale: 0 }}
            ...
            >
                ...
                <div className="relative bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 p-5 rounded-full shadow-[0_0_30px_rgba(59,130,246,0.4)] border border-white/40 backdrop-blur-xl">
                    <MessageSquare size={28} className="text-white group-hover:scale-110 transition-all" />
                </div>
            </motion.button>
        );
    }

    // Standalone open state: draggable/resizable window
    return (
        <Rnd
            default={{
            ...
            >
                ...
                <div className="chat-header cursor-move px-5 py-4 bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 flex items-center justify-between border-b border-gray-200">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-white font-bold text-[15px]">PMA Smart System</h2>
                            <p className="text-blue-100 text-[11px]">AI Production Assistant</p>
                        </div>
                    </div>
                    <div className="flex gap-2">
                        <button onClick={clearHistory} className="p-2 rounded-lg bg-white/20 hover:bg-white/30 transition-all">
                            <Trash2 size={16} className="text-white" />
                        </button>
                        <button onClick={() => setIsOpen(false)} className="p-2 rounded-lg bg-white/20 hover:bg-white/30 transition-all">
                ...
                {/* Input form */}
                <form onSubmit={handleSend} className="p-4 border-t border-gray-200 bg-gray-50 backdrop-blur-xl">
